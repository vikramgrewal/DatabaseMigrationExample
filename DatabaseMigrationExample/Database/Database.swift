...
struct Database {
    ...
    static let sharedInstance = Database()?.db
...
extension Database {
    ...
    static func dropEntireDatabase() throws {
        
        let movie = Table("Movies")
        let migrations = Table("schema_migrations")
        do {
            let connection = try Connection(Database.storeURL().absoluteString)
            
            try connection.run(migrations.drop(ifExists: true))
            try connection.run(movie.drop(ifExists: true))
        }
    }
}
